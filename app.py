...
# data = pd.read_csv('test_df.csv').set_index('SK_ID_CURR')
# train_df.csv pd.read_csv('C:/Users/Fane0763/OpenClassroom/OC Projet 7/bases/application_test.csv')
# data = pd.concat([df_id, data], axis=1)

# data_train = pd.read_csv('train_df.csv').set_index('SK_ID_CURR')  # train_df.csv
# X_train = pd.read_csv('X_train.csv').set_index('SK_ID_CURR')
...
def shap_values_local(client_id: int):
    ...
    # return {
    #     'shap_values'  : shap_val.tolist(),
    #     'base_value'   : base_value, # shap_val.base_values.item(),
    #     'data'         : client_data.values.tolist(),
    #     'feature_names': client_data.columns.tolist()
    # }
